import Lotto from "../src/model/entity/Lotto.js";
import LottoDraw from "../src/model/entity/LottoDraw.js";
import LottoEvaluator from "../src/model/service/LottoEvaluator.js";
import { RANK } from "../src/constants.js";

describe("LottoEvaluator 클래스 테스트", () => {
  describe("evaluateTicket 메서드 테스트", () => {
    test("3개의 숫자가 일치할 경우 5등을 반환한다.", () => {
      ...
      expect(LottoEvaluator.evaluateTicket(lottoDraw, ticket)).toBe(RANK.FIFTH);
    });

    test("4개의 숫자가 일치할 경우 4등을 반환한다.", () => {
      const ticket = new Lotto([1, 2, 3, 4, 5, 6]);
      const lottoDraw = new LottoDraw([1, 2, 3, 4, 43, 44], 45);
      expect(LottoEvaluator.evaluateTicket(lottoDraw, ticket)).toBe(RANK.FOURTH);
    });

    test("5개의 숫자가 일치할 경우 3등을 반환한다.", () => {
      const ticket = new Lotto([1, 2, 3, 4, 5, 6]);
      const lottoDraw = new LottoDraw([1, 2, 3, 4, 5, 44], 45);
      expect(LottoEvaluator.evaluateTicket(lottoDraw, ticket)).toBe(RANK.THIRD);
    });

    test("5개의 숫자와 보너스 번호가 일치할 경우 2등을 반환한다.", () => {
      const ticket = new Lotto([1, 2, 3, 4, 5, 6]);
      const lottoDraw = new LottoDraw([1, 2, 3, 4, 5, 45], 6);
      expect(LottoEvaluator.evaluateTicket(lottoDraw, ticket)).toBe(RANK.SECOND);
    });

    test("6개의 숫자가 일치하는 경우 1등을 반환한다.", () => {
      const ticket = new Lotto([1, 2, 3, 4, 5, 6]);
      const lottoDraw = new LottoDraw([1, 2, 3, 4, 5, 6], 7);
      expect(LottoEvaluator.evaluateTicket(lottoDraw, ticket)).toBe(RANK.FIRST);
    });
  });
